...
input_frame = Frame(win,width=312, height=50, bd=0, highlightbackground="black", highlightthickness=2)
...
btns_frame = Frame(win, width=312,height=272.5, bg= "#00ffe1")
btns_frame.pack()

clear = Button(btns_frame, text="Clear", command= lambda: bt_clear()).grid(row=0,column=0, columnspan=3, padx=1,pady=1)
...
multiply = Button(btns_frame, text = "x", command = lambda: btn_click("*")).grid(row = 1, column = 3, padx = 1, pady = 1)
...
equals = Button(btns_frame, text = "=", command = lambda: bt_equal()).grid(row = 4, column = 3, padx = 1, pady = 1)
